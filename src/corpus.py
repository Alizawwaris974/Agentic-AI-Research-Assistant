CURATED_CORPUS = [
    ...
    {"id": "doc28", "title": "Recursion Limits in Agent Loops",
     "text": "Agent frameworks impose a recursion or step limit on the reason-act loop so that a model that keeps calling tools without converging on an answer is forced to stop and return its best answer so far, rather than looping indefinitely."},
    {"id": "doc29", "title": "Checkpointing and Session Memory",
     "text": "A checkpointer persists an agent's running state keyed by a thread/session id, so a multi-turn conversation can resume exactly where it left off — including prior tool results — without the caller having to resend the full history each turn."},
    {"id": "doc30", "title": "CORS in Web APIs",
     "text": "Cross-Origin Resource Sharing (CORS) headers tell a browser which external origins may call an API from client-side JavaScript. Without permissive CORS configuration, a browser-based chat UI hosted on a different domain than the API will have its requests blocked."},
    {"id": "doc31", "title": "Webhooks and Workflow Automation (n8n)",
     "text": "Low-code automation tools like n8n let a 'trigger' node (e.g. an incoming chat message) fan out to HTTP request nodes that call external APIs, then route the response back into a chat widget — a common pattern for wrapping a custom backend in a no-code front door."},
    {"id": "doc32", "title": "Tunnelling Local Servers (ngrok)",
     "text": "A tunnelling service like ngrok exposes a server running on a local or sandboxed machine (such as a Kaggle kernel) at a temporary public HTTPS URL, which is essential when the development environment itself has no stable public address."},
    {"id": "doc33", "title": "Citations and Grounded Answers",
     "text": "A grounded answer explicitly attributes claims to the specific retrieved passage or search result that supports them, which both improves user trust and makes it possible to verify or challenge any individual claim."},
    {"id": "doc34", "title": "Temperature and Sampling",
     "text": "Temperature controls the randomness of next-token sampling: low values make output more deterministic and focused, while higher values increase diversity at the cost of consistency — agentic tool-routing decisions typically use low or zero temperature."},
    {"id": "doc35", "title": "Embedding Drift and Re-indexing",
     "text": "If a corpus is re-embedded with a different embedding model, old and new vectors are not comparable, so a vector store must be fully rebuilt rather than incrementally mixed when switching embedding models."},
    {"id": "doc36", "title": "Glassmorphism vs. Purposeful UI Design",
     "text": "Glassmorphism (frosted-glass panels, blur, soft gradients) is one stylistic choice among many for a chat interface; for a research-tool product, a design language that visually echoes the subject matter — citations, margins, footnotes — can communicate the tool's purpose more directly than a generic dark-glass aesthetic."},
]
